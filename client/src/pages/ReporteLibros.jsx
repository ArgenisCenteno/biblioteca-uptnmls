...
const ReporteLibros = () => {
  ...
  const handleGenerateReport = async () => {
    try {
      const response = await axios.get(`${apiKey}/api/borrow-book/getBorrowBooksByDateParam/${selectedOption}`);
      const data = response.data;
  
      // Crear un nuevo documento PDF
      const pdf = new jsPDF();
      pdf.text('Reporte de Prestamos de Libros', 10, 10);
      ...
      const columnWidth = [10, 25, 25, 20, 20, 20, 15, 25, 25, 30, 30];
...
const rows = data.map((item) => [
    ...
    item.apellido_solicitante,
    item.cedula_solicitante, 
    item.ocupacion_solicitante,
    item.codigo_ejemplar,
    item.titulo,
  ]);
