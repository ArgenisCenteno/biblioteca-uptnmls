...
const BorrowBook = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${apiKey}/api/books/getAllBooksAndCopies`);
  ...
  const fetchCopies = async (id_libro) => {
    try {
      const response = await axios.get(`${apiKey}/api/copies/getCopies/${id_libro}`);
      const booksData = response.data;
  ...
  const handleCopyChange = (event) => {
    ...
    if (selectedDataArray.length <= 3) {
      ...
      const selectedCopyObj = copies.find((copy) => copy.id_ejemplar === Number(selectedCopyId));

      if (selectedCopyObj) {
        // Encuentra el libro relacionado basado en el id_libro del ejemplar seleccionado
        const relatedBook = books.find((book) => book.id_libro === selectedCopyObj.id_libro);

        if (relatedBook) {
          // Crea un objeto con los datos seleccionados
          const selectedData = {
            id_ejemplar: selectedCopyObj.id_ejemplar,
  ...
  const columns = [
    { field: 'titulo', headerName: 'Título', width: 220 },
    { field: 'codigo_ejemplar', headerName: 'Ejemplar', width: 120 },
  ...
  const sendRequest = async () => {
    if (selectedDataArray.length > 0 && selectedBorrowers.length > 0) {
      try {
        ...
        const response = await axios.post(`${apiKey}/api/borrow-book/createBorrowBook`, {
  ...
  return (
    <Layout>
      <h2 className='m-3 p-3'>Prestar Libro</h2>
      <div className='row m-3 p-3'>
       
        <div className='col-5'>
  <label className='form-label mt-3'>Seleccionar Libro:</label>
  <Select
              ...
              <select className='form-control' value={selectedCopy} onChange={handleCopyChange}>
                ...
                {copies.map((copy) => (
                  <option key={copy.id_ejemplar} value={copy.id_ejemplar}>
                    Ejemplar {copy.TituloDelLibro} {copy.codigo_ejemplar}
                  </option>
                ))}
              </select>
            </div>
  ...
  <div className='col-7'>
    ...
    <div style={{ width: '99%' }}>
      <DataGrid
        ...
        pageSize={5} // Puedes ajustar el tamaño de página según tus necesidades
        rowsPerPageOptions={[5, 10, 20]} // Opciones de tamaño de página
